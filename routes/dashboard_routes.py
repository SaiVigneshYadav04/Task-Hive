    ...
    try:
        ...
        if existing:
            if not existing.status or existing.status not in ["pending", "accepted", "rejected"]:
                ...
                db.session.commit()
                return jsonify({"message": "Application recovered and submitted successfully!"})
            else:
        ...
        db.session.rollback()
        print(f"CRASH in apply_task: {e}")
        return jsonify({"error": "Server error. Check VS Code terminal!"}), 500
    ...
    if category == "all":
        ...
        for worker in top_workers:
            ...
            leaderboard_data.append({
                "name": worker.name,
                "tasks_completed": worker.tasks_completed or 0,
                "rating": avg_rating,
...
def cancel_task(task_id):
    ...
    if str(task.posted_by) != str(session["user_id"]):
        return jsonify({"error": "Only the task poster can delete this."}), 403
