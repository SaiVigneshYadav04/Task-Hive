...
with app.app_context():
    db.create_all()
    # Migration helper: Check if columns exist, add if missing
    try:
    ...
    try:
        from sqlalchemy import text
        db.session.execute(text("ALTER TABLE notification ADD COLUMN type VARCHAR(50)"))
        db.session.commit()
        print("Database Migration: Added type column to notification.")
    except Exception:
        db.session.rollback()
